...
function checkBounds($val, $var, $low, $high){
  if ($val < $low || $val > $high || !isInteger($val) )
...
if ($sec == "general") {
  ...
  if ($var == "thumbsize") {
    $prevThumbSize = getthumbsize($_SESSION[pictures]);
    checkBounds($val, $var, 10, 400);
    if ($prevThumbSize != $val)
    $msg = "It is recommended that you <a href=# onclick=\"confirmation('regenerate all thumbnails', '$script?mode=regeneratethumbs')\">re-generate your thumbnails</a>";
  }
